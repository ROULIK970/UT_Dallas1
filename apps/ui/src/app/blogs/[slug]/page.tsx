    ...
    return (
        <div className='p-10'>
            <div className=" mx-auto">
                ...
                <p className="text-gray-600 text-center text-sm mb-[12px]">{blog.blogDescription}</p>
                <p className='text-center mb-[12px]'>Dr Sarah Chen</p>
                <Image src={blog.img} width={1601} height={728} alt="blog-image" className='mx-auto' />
                <div className='flex md:flex-row flex-col gap-[56px] md:mt-[100px] mt-[10px]'>
                    <div className='flex flex-col gap-5'>
                        ...
                        <div className='flex justify-between items-center'>
                            <div className='hidden md:flex'>
                                <Image src='/arrow-left-line.svg' alt='arrow-left-line' width={24} height={24}/>
                                <p className='text-[20px] text-[#1D2A49]'>Back to Home</p>
                            </div>
                            <div className='flex gap-2 justify-between md:justify-end items-center  w-full'>
                                <button className='py-[8px] items-center px-[10px] md:py-[16px] md:px-[20px] flex rounded-[9px] border border-black shadow-[0_0_10px_0_rgba(0,0,0,0.1)]'>
                                    <p className='md:text-[20px] text-[10px]'>Share</p>
                                    <Image src='/share-icon.svg' alt='share' width={24} height={24}/>
                                </button>
                                
                                <button className='py-[8px] items-center px-[10px] md:py-[16px] md:px-[20px] text-[10px] md:text-[20px] text-[#FFFFFF] rounded-[9px] bg-[#3B3098] shadow-[0_0_10px_0_rgba(0,0,0,0.1)]'>Download PDF/ Doc</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='flex-1'>
                        <h1 className='text-[#1D2A49] text-[24px] md:text-[36px] font-medium'>Latest Blogs</h1>
                        {latestBlogs.map((blog, index) => (
                            <div key={index} className='rounded-[10px] p-2 md:p-5 mt-2 border md:h-[195px] md:w-[461px] border-[rgba(68,98,169,0.2)] shadow-[0_4px_20px_0_rgba(153,86,236,0.08)]'>
                                <p className='md:py-[12px] md:px-[16px] py-[9px] px-[12px] text-left rounded-[4px] bg-[rgba(68,98,169,0.1)] inline-flex'>{blog.blogTitle}</p>
                                <p className='text-[#1D2A49] text-[12px] md:text-[20px] font-semibold'>{blog.blogDescription}</p>
                                <div className='flex'>
                                    <p className='md:text-[16px] text-[12px]'>Read more</p>
                                    <Image src='/arrow-right-up.svg' alt='arrow-right-up' height={18} width={18}/>
                                </div>
                            </div>
                        ))}
