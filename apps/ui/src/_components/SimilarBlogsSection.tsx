          ...
          <Link href={`/blogs/${blog.slug}`} key={index} className='rounded-[10px] border mb-[20px] border-[rgba(68,98,169,0.2)] bg-white shadow-[0_4px_20px_0_rgba(153,86,236,0.08)] transform transition-all duration-300 ease-in-out grayscale hover:grayscale-0 hover:scale-105'>
            ...
            <div className='p-5'>
              <div className="col-span-1">
                <p className='font-semibold md:py-[12px] md:px-[16px] py-[10px] px-[15px] text-left rounded-[4px] bg-[rgba(68,98,169,0.1)] inline-flex'>{blog.title}</p>
              </div>

              <div className='flex text-[16px] justify-between mt-[10px]'>
                <div className='flex items-center gap-1.5 '>
                  <Image src='/Publications_logo.png' alt='publications-logo' width={16} height={16}/>
                  <p className='md:text-[16px] text-[12px]'>{blog.publishedAt}</p> 
                  </div>
                <div className='flex items-center gap-1.5'>
                  <span>.</span>
              ...
              <p className='mt-[10px] text-[10px] md:text-[20px] text-[#1D2A49] font-medium'>{blog.blogDescription}</p>
            </div>
